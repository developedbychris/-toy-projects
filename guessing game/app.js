        ...
        if(userChoice === randNum && guessCount === 0){
            guessResults.innerHTML = `You guessed: ${userChoice}.<br>
             The number was ${randNum}. It took you ${guessCount + 1} try.<br>
              You Win!<br> <span id="resPrompt">Restarting. . .</span>`, guessResults.style.color='green', guessCount = 0, fTry++, 
              fTryArea.innerHTML = `Number of first tries: ${fTry}`, numRight++, e.target.style.color ='purple' , e.target.style.outlineColor ='purple', 
              randNum = Math.floor(Math.random() * 6) + 1, gameWinTimeout(), boxesDisabled(),
              document.querySelector('#resPrompt').style.color ="white"
        } 
